...
export function ContentManagerTab() {
  const { content, loading, saveContent, restoreDefault, getVersions, refresh } = useContent();
  ...
  const sections = useMemo(() => {
    ...
    Object.values(content).forEach(c => set.add(c.section));
  ...
  const getDraft = useCallback((key: string) => {
    ...
    return drafts[key] ?? { en: row?.value_en ?? '', te: row?.value_te ?? '' };
  }, [content, drafts]);

  const setDraft = useCallback((key: string, lang: 'en' | 'te', val: string) => {
    setDrafts(prev => ({
      ...prev,
      [key]: { ...prev[key] ?? { en: content[key]?.value_en ?? '', te: content[key]?.value_te ?? '' }, [lang]: val },
    }));
  }, [content]);
  ...
  const isDirty = useCallback((key: string) => {
    ...
    const d = drafts[key];
    if (!d) return false;
    return d.en !== (row?.value_en ?? '') || d.te !== (row?.value_te ?? '');
  }, [content, drafts]);

  const dirtyCount = useMemo(() => Object.keys(content).filter(isDirty).length, [content, isDirty]);

  async function handleSave(key: string) {
    setSavingKey(key);
    setSaveError(null);
    const d = drafts[key];
    if (!d) { setSavingKey(null); return; }
    try {
  ...
  async function handleSaveAll() {
    ...
    try {
      for (const key of dirty) {
        const d = drafts[key];
        if (d) await saveContent(key, d.en, d.te);
      }
      setDrafts({});
      setSavedKey('__all__');
      setTimeout(() => setSavedKey(null), 2500);
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Failed to save some content. Please try again.');
    } finally {
      setSavingKey(null);
    }
  }
  ...
  async function handleRestore(key: string) {
    if (!confirm('Restore this content to its first saved version?')) return;
    setRestoringKey(key);
    try {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-maroon-500 dark:text-cream/60">Loading content...</span>
      </div>
    );
  }
    ...
    {mapsSaved && (
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
        ...
        <div className="flex items-center gap-2">
          {dirtyCount > 0 && (
            <Badge color="gold">{dirtyCount} unsaved</Badge>
          )}
          <button
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(item => {
            ...
            return (
              <Card key={item.key} className={dirty ? 'ring-2 ring-saffron-300' : ''}>
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge color="saffron">{item.section}</Badge>
                    <span className="text-sm font-semibold text-maroon-800 dark:text-gold-200">{item.label}</span>
                    <code className="text-xs text-maroon-400 dark:text-cream/50 font-mono">{item.key}</code>
                    {dirty && <Badge color="gold">Unsaved</Badge>}
                    {isSaved && (
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] uppercase tracking-wide font-semibold text-maroon-500 dark:text-cream/50">English</label>
                      {item.content_type === 'rich' ? (
                        <textarea
                          value={d.en}
                          onChange={e => setDraft(item.key, 'en', e.target.value)}
                    ...
                    <div>
                      <label className="text-[10px] uppercase tracking-wide font-semibold text-maroon-500 dark:text-cream/50">Telugu</label>
                      {item.content_type === 'rich' ? (
                        <textarea
                          value={d.te}
                          onChange={e => setDraft(item.key, 'te', e.target.value)}
      ...
      {historyKey && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-maroon-950/50 backdrop-blur-sm" onClick={() => setHistoryKey(null)}>
          <div className="bg-white dark:bg-maroon-900 rounded-2xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-saffron-100 dark:border-maroon-800">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display text-base font-semibold text-maroon-800 dark:text-gold-200">Version History</h3>
                <code className="text-xs text-maroon-400 font-mono">{historyKey}</code>
              </div>
              <button onClick={() => setHistoryKey(null)} className="btn-ghost h-8 w-8 p-0">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="overflow-y-auto p-4 space-y-2">
              {versions.length === 0 ? (
                <p className="text-center text-maroon-500 dark:text-cream/60 py-8">No version history available.</p>
              ) : (
                versions.map((v, i) => (
                  <div key={v.id} className="rounded-xl border border-saffron-100 dark:border-maroon-800 p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge color={i === 0 ? 'gold' : 'gray'}>{i === 0 ? 'Latest' : `v${versions.length - i}`}</Badge>
                      <span className="text-xs text-maroon-400 dark:text-cream/50">
                        {new Date(v.changed_at).toLocaleString()}
                      </span>
                    </div>
                    <p className="text-sm text-maroon-700 dark:text-cream/80 mt-1">EN: {v.value_en || '—'}</p>
                    <p className="text-sm text-maroon-700 dark:text-cream/80 font-telugu">TE: {v.value_te || '—'}</p>
                  </div>
                ))
              )}
            </div>
