  ...
  return (
    <div
      className={`glass-card p-5 ${
        hover
          ? 'transition-all duration-300 hover:-translate-y-1 hover:shadow-glow-saffron'
          : ''
      } ${className}`}
    >
      {children}
...
export function Badge({
  ...
  children: ReactNode;
  color?: 'saffron' | 'gold' | 'maroon' | 'lotus' | 'green' | 'gray';
  className?: string;
}) {
  const colors: Record<string, string> = {
    saffron:
      'bg-saffron-100 text-saffron-700 dark:bg-saffron-900/40 dark:text-saffron-200',
    gold:
      'bg-gold-100 text-gold-700 dark:bg-gold-900/40 dark:text-gold-200',
    maroon:
      'bg-maroon-100 text-maroon-700 dark:bg-maroon-900/40 dark:text-maroon-200',
    lotus:
      'bg-lotus-100 text-lotus-700 dark:bg-lotus-900/40 dark:text-lotus-200',
    green:
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
      ...
      {message && (
        <p className="text-sm text-maroon-500 dark:text-cream/60 max-w-sm">
          {message}
        </p>
      )}
...
export function StatCard({
  ...
  value: string;
  icon?: ReactNode;
  accent?: 'saffron' | 'gold' | 'maroon' | 'lotus';
  sub?: string;
}) {
    ...
    <Card hover className="relative overflow-hidden">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-wide text-maroon-500 dark:text-cream/60">
            {label}
          </p>

          <p className="mt-1 font-display text-2xl md:text-3xl font-bold text-maroon-800 dark:text-gold-200 truncate">
            {value}
          </p>
