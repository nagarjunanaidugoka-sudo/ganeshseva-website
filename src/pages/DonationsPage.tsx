...
function validate(f: FormState): Record<string, string> {
  ...
  if (f.transaction_id && f.transaction_id.length > 100) {
    errs.transaction_id = 'Transaction ID is too long';
  }
...
function sortDonations(data: Donation[], sortBy: SortOption): Donation[] {
  return [...data].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'name_asc':
        return a.donor_name.localeCompare(b.donor_name, undefined, {
          sensitivity: 'base',
        });
    }
  });
...
export function DonationsPage() {
  ...
  // DEFAULT = RECEIPT NUMBER HIGH TO LOW
  ...
  function pushToast(type: Toast['type'], msg: string) {
    ...
    setTimeout(() => {
      setToasts(ts => ts.filter(tst => tst.id !== id));
    }, 4000);
  }
  ...
  const exportData = useMemo(() => {
    const base = filtersApplied
      ? filtered
      : donations.filter(d => d.approval_status === 'approved');
  ...
  function getDonorVillage(d: Donation): string {
    return donors.find(
      dn =>
        dn.name === d.donor_name &&
        dn.father_name === (d.father_name ?? '')
    )?.village ?? '';
  }
  ...
  /*
   * Checks whether the selected donor has already been used in a donation.
   *
   * IMPORTANT:
   * While EDITING, the current donation itself is ignored.
   */
  const existingDonationsForDonor = useMemo(() => {
    const name = form.donor_name.trim().toLowerCase();
    const father = form.father_name.trim().toLowerCase();
    ...
    return donations.filter(d => {
      ...
      const donorName = d.donor_name.trim().toLowerCase();
      const donorFather = (d.father_name ?? '').trim().toLowerCase();

      if (donorName !== name) return false;

      // If father name is entered, use it as an additional match.
      if (father && donorFather && donorFather !== father) return false;

      return true;
    });
  }, [donations, form.donor_name, form.father_name, editing]);

  const donorAlreadyAdded = existingDonationsForDonor.length > 0;

  function openAdd() {
    setEditing(null);
    setSelectedDonor(null);

    const nextNo = `GSV-${new Date().getFullYear()}-${String(
      donations.length + 1
    ).padStart(4, '0')}`;
  ...
  async function confirmAddNewDonor() {
    ...
    // Prevent duplicate donor master records too.
    const duplicateDonor = donors.find(d => {
      const sameName =
        d.name.trim().toLowerCase() === newDonorName.trim().toLowerCase();

      const enteredFather = newDonorFather.trim().toLowerCase();
      const existingFather = (d.father_name ?? '').trim().toLowerCase();

      return sameName && (!enteredFather || !existingFather || enteredFather === existingFather);
    });

    if (duplicateDonor) {
      setSelectedDonor(duplicateDonor);

      setForm(f => ({
        ...f,
        donor_name: duplicateDonor.name,
        father_name: duplicateDonor.father_name ?? '',
        phone: duplicateDonor.phone || f.phone,
      }));

      pushToast(
        'error',
        `"${duplicateDonor.name}" is already in the donor master list`
      );
    ...
    try {
      ...
      setForm(f => ({
        ...
        father_name: newDonor.father_name,
        phone: newDonor.phone || f.phone,
      }));

      pushToast(
        'success',
        `Donor "${newDonor.name}" added to master list`
      );
      ...
      pushToast(
        'error',
        err instanceof Error ? err.message : 'Failed to add donor'
      );
    } finally {
  ...
  async function handleSubmit(e: FormEvent) {
     ...
     * This prevents a donor from being added even if the warning was
     * bypassed or the donor name was typed manually.
     */
    if (!editing && donorAlreadyAdded) {
      pushToast(
        'error',
        'This donor is already added. Please select the existing donation or use a different donor.'
      );
      return;
    }
    ...
    try {
      if (editing) {
        ...
        await logAudit('donations', editing.id, 'update', {
          old: editing,
          new: row,
        });
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      await logAudit('donations', deleteTarget.id, 'delete', {
  ...
  if (loading) {
    return <Loader label={t('common.loading', lang)} />;
  }
    ...
    <div className="space-y-8 pb-10">
      <div className="fixed top-4 right-4 z-[60] space-y-2">
        {toasts.map(tst => (
          ...
          >
            ...
            <button
              onClick={() =>
                setToasts(ts => ts.filter(x => x.id !== tst.id))
              }
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={formatINR(totalDonations(donations))}
        ...
        <StatCard
          ...
          value={formatINR(
            donations.length
              ? totalDonations(donations) / donations.length
              : 0
          )}
          icon={<Marigold className="w-6 h-6" />}
          accent="lotus"
        />
      ...
      <Card>
        <div className="flex flex-col md:flex-row gap-3">
          ...
          >
            <option value="receipt_desc">
              Receipt No. 鈥� High to Low
            </option>
            <option value="receipt_asc">
              Receipt No. 鈥� Low to High
            </option>
            ...
            <option value="amount_desc">
              Amount 鈥� High to Low
            </option>
            <option value="amount_asc">
              Amount 鈥� Low to High
            </option>
                ...
                <div className="absolute right-0 top-full mt-2 z-20 bg-white dark:bg-maroon-900 rounded-xl shadow-lg border border-saffron-200 dark:border-maroon-700 py-1 min-w-[170px]">
                  {exportData.length === 0 ? (
                    ...
                    <>
                      <button
                        onClick={exportXLSX}
                        className="w-full px-4 py-2 text-sm text-left hover:bg-saffron-50 dark:hover:bg-maroon-800 text-maroon-700 dark:text-cream flex items-center gap-2"
                      >
      ...
      <Card className="p-0 overflow-hidden">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            message="Try adjusting your search or filters, or add a new donation."
          />
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((d, i) => (
                  <tr
                    key={d.id}
                    className={`border-t border-saffron-100 dark:border-maroon-800 hover:bg-saffron-50/60 dark:hover:bg-maroon-800/40 transition ${
                      i % 2 === 0
                        ? ''
                        : 'bg-cream/40 dark:bg-maroon-950/30'
                    }`}
                  >
                    <td className="px-4 py-3 font-mono text-xs text-maroon-600 dark:text-gold-300">
                      {d.receipt_no}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-maroon-800 dark:text-cream truncate">
                            {lang === 'te' && d.donor_name_te
                              ? d.donor_name_te
                              : d.donor_name}
                          </p>
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell text-maroon-600 dark:text-cream/70">
                      {d.category}
                    </td>

                    <td className="px-4 py-3 hidden md:table-cell">
                      <Badge color={methodColor[d.method]}>
                        {d.method}
                      </Badge>
                    </td>

                    <td className="px-4 py-3 hidden md:table-cell">
                      <Badge color={statusColor[d.payment_status]}>
                        {d.payment_status}
                      </Badge>
                    </td>

                    <td className="px-4 py-3 hidden lg:table-cell text-maroon-500 dark:text-cream/60">
                      {formatDate(d.date)}
                    </td>

                    <td className="px-4 py-3 text-right font-bold text-saffron-600 dark:text-saffron-300">
                      {formatINR(d.amount)}
                    </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            ...
                            className="btn-ghost h-8 w-8 p-0 rounded-lg text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30"
                            aria-label="Delete"
                          >
              ...
              <tfoot>
                <tr className="bg-saffron-100/60 dark:bg-maroon-800/60 border-t-2 border-saffron-300 dark:border-maroon-700">
                  <td
                    colSpan={isAdmin ? 6 : 5}
                    className="px-4 py-3 font-semibold text-maroon-700 dark:text-gold-200"
                  >
                    Total
                  </td>

                  <td className="px-4 py-3 text-right font-bold text-maroon-800 dark:text-gold-200">
                    {formatINR(total)}
                  </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {selectedDonor && (
              <div className="mt-1.5 flex flex-wrap gap-2 text-xs text-maroon-500 dark:text-cream/60">
                {selectedDonor.father_name && (
                  <span>S/o {selectedDonor.father_name}</span>
                )}
                {selectedDonor.village && (
                  <span>路 {selectedDonor.village}</span>
                )}
                {selectedDonor.phone && (
                  <span>路 {selectedDonor.phone}</span>
                )}
              </div>
            )}

            {/* ALREADY ADDED WARNING */}
            {donorAlreadyAdded && !editing && (
              <div className="mt-3 rounded-xl border-2 border-amber-300 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-700 p-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                      This donor already has a donation record.
                    </p>

                    <div className="mt-2 space-y-1">
                      {existingDonationsForDonor.map(existing => (
                        <div
                          key={existing.id}
                          className="text-xs text-amber-800 dark:text-amber-200"
                        >
                          <strong>{existing.receipt_no}</strong>
                          {' 鈥� '}
                          {formatINR(existing.amount)}
                          {' 鈥� '}
                    ...
                    <p className="text-xs text-amber-700 dark:text-amber-300 mt-2">
                      Please check the existing donation before saving another
                      record for this donor.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setForm({
                    ...form,
                    donor_name_te: e.target.value,
                  })
                }
                className="input"
                placeholder="喟む眴喟侧眮喟椸眮 喟眹喟班眮 (optional)"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="label">
                Amount (鈧�) <span className="text-red-500">*</span>
              </label>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              disabled={saving || donorAlreadyAdded && !editing}
      ...
      <Modal
        ...
        title="Add New Donor to Master List"
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3 mb-2">
            ...
            <p className="text-sm text-maroon-500 dark:text-cream/60">
              This donor will be saved to the master list and linked to this
              donation.
            </p>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={confirmAddNewDonor}
              disabled={savingDonor || !newDonorName.trim()}
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-medium text-maroon-800 dark:text-gold-200">
